...
ggplot(plotdata,aes(Date,`Target rate`))+
  ...
  geom_vline(xintercept = as.Date("2020-03-23"),linewidth=1,linetype='dashed',color=col[1])+
  geom_line(linewidth=1.5,show.legend = F,color=col[2])+
...
ggplot(plotdata,aes(Date,BankRate))+
  ...
  geom_vline(xintercept = as.yearmon("June 1994"),linewidth=1,linetype='dashed',color=col[1])+
  geom_line(linewidth=1.5,show.legend = F)+
